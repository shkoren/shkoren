...
rawDataClean = rawData %>% clean_names() %>% filter(master == 'IsMasterProtein') %>% filter(x_unique_peptides > 0) %>% select(c(6:7, 44:63, 194:215)) %>% na.omit() %>%  mutate(Symbol = unlist(rm_between(description, "GN=", " PE", extract=TRUE))) %>% select(Symbol, everything()) %>% rename(., Uniprot = accession)
